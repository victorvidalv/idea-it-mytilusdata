...
// Obtener información del centro (API)
router.get('/:id/info', isAuthenticated, async (req, res) => {
    try {
        ...
        const { id, name, location, latitude, longitude, address } = center;
        res.json({ id, name, location, latitude, longitude, address });
    } catch (err) {
        res.status(500).json({ error: 'Error al obtener la información del centro' });
    }
});

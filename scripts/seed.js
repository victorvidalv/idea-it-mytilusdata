...
function poblar(userId) {
    ...
    if (tipos.length === 0 || origenes.length === 0) {
        console.error('❌ No hay tipos de registro u orígenes de datos en la DB. Crea algunos primero.');
        process.exit(1);
    }
    ...
    const centro2 = insertLugar.run('Estero Castro - Línea Sur', -42.480, -73.765, userId);
    ...
    for (const c of ciclosData) {
        ...
        console.log(`   🔄 Ciclo: "${c.nombre}" (ID: ${result.lastInsertRowid}) → Centro ${c.centroId}`);
    }
    ...
    for (const ciclo of cicloIds) {
        ...
        for (let i = 0; i <= numMuestreos; i++) {
            ...
            if (tipoMap['TALLA_LONGITUD']) {
                const talla = 5 + 60 * (1 / (1 + Math.exp(-8 * (progreso - 0.4)))) + (Math.random() * 3 - 1.5);
                insertMedicion.run(
                    Math.round(talla * 100) / 100, ts, ciclo.id, ciclo.centroId, userId,
                    tipoMap['TALLA_LONGITUD'].id, defaultOrigen.id,
            ...
            if (tipoMap['PESO_VIVO']) {
                ...
                insertMedicion.run(
                    Math.round(Math.max(peso, 0.5) * 100) / 100, ts, ciclo.id, ciclo.centroId, userId,
                    tipoMap['PESO_VIVO'].id, labOrigen.id, null
                );
                totalMediciones++;
            }
            ...
            if (tipoMap['TEMPERATURA_AGUA']) {
                ...
                insertMedicion.run(
                    Math.round(temp * 10) / 10, ts, null, ciclo.centroId, userId,
                    tipoMap['TEMPERATURA_AGUA'].id, sensorOrigen.id, null
                );
                totalMediciones++;
            }
            ...
            if (tipoMap['SALINIDAD'] && i % 2 === 0) {
                ...
                insertMedicion.run(
                    Math.round(salinidad * 10) / 10, ts, null, ciclo.centroId, userId,
                    tipoMap['SALINIDAD'].id, sensorOrigen.id, null
                );
                totalMediciones++;
            }
            ...
            if (tipoMap['OXIGENO_DISUELTO'] && i % 3 === 0) {
                ...
                insertMedicion.run(
                    Math.round(od * 100) / 100, ts, null, ciclo.centroId, userId,
                    tipoMap['OXIGENO_DISUELTO'].id, sensorOrigen.id,
            ...
            if (tipoMap['CLOROFILA_A'] && i % 4 === 0) {
                ...
                insertMedicion.run(
                    Math.round(clor * 100) / 100, ts, null, ciclo.centroId, userId,
                    tipoMap['CLOROFILA_A'].id, labOrigen.id,

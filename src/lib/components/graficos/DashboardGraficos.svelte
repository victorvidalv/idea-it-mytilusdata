<script lang="ts">
	...
	// --- Estado de filtros ---
	let selectedUserId: string | null = isInvestigador ? null : 'all';
		...
		const porTipo = tiposActivos
			.map((tipo: TipoRegistro) => {
				...
				return {
					...
					promedio:
						Math.round(
							(valores.reduce((a: number, b: number) => a + b, 0) / valores.length) * 100
						) / 100,
...
<div class="space-y-6">
	...
	<div class="animate-fade-up flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
		...
		<div class="sm:self-end">
			{#if isInvestigador && data.usuarios}
				<select
		...
		<Card.Root class="border-border/50">
			<Card.Header class="pb-4">
				<Card.Title class="font-display text-lg"
					>{isInvestigador ? 'Sub-filtros Espaciales/Técnicos' : 'Filtros'}</Card.Title
				>
		...
		<Card.Root class="overflow-hidden border-border/50">
			<Card.Header>
				<div class="flex items-center justify-between">
					<div>
						<Card.Title class="font-display text-lg"
							>Evolución Temporal{isInvestigador ? ' Global' : ''}</Card.Title
						>
					...
					<Card.Root class="border-border/50 transition-colors duration-300 hover:border-border">
						<Card.Header class="flex flex-row items-center justify-between space-y-0 pb-2">
							<Card.Title class="font-body text-sm font-medium text-muted-foreground"
								>{isInvestigador ? 'Total Mediciones Visualizadas' : 'Total Mediciones'}</Card.Title
							>
							<div

...
<script lang="ts">
    ...
    import { formatearFormulaSigmoidal, type CurvaUsada, type Metadatos } from './ProyeccionComponentTypes';
		...
		<div class="rounded-lg border border-border/50 bg-secondary/20 p-3 space-y-2">
			<div class="flex items-center gap-2">
				...
				<span class="text-xs font-semibold">
					{curvaUsada.esCurvaLocal ? 'Ajuste local (Levenberg-Marquardt)' : 'Ajuste (análogo dinámico escalado)'}
				</span>
			</div>
			<p class="text-[10px] text-muted-foreground pl-5">
				{curvaUsada.esCurvaLocal
					? 'Optimización no lineal directa de los 3 parámetros.'
					: 'Rescalado analítico de L basado en el análogo más similar.'}
			</p>
			...
			<div class="grid grid-cols-3 gap-2 pl-5">
				<div><p class="text-[10px] text-muted-foreground">L</p><p class="font-mono text-xs font-semibold">{curvaUsada.parametros.L.toFixed(2)} mm</p></div>
				<div><p class="text-[10px] text-muted-foreground">k</p><p class="font-mono text-xs font-semibold">{curvaUsada.parametros.k.toFixed(4)}</p></div>
				<div><p class="text-[10px] text-muted-foreground">x₀</p><p class="font-mono text-xs font-semibold">d {curvaUsada.parametros.x0.toFixed(1)}</p></div>
			</div>
		</div>
	{/if}
	...
	{#if curvaReferencia}
		<div class="rounded-lg border border-border/50 bg-secondary/10 p-3 space-y-2">
			<div class="flex items-center gap-2">
				...
				<span class="text-xs font-semibold">Análogo: {curvaReferencia.codigoReferencia}</span>
			</div>
			<div class="rounded-md bg-background/60 p-2 text-center">
				<p class="font-mono text-[11px]">{formatearFormulaSigmoidal(curvaReferencia.parametros, 'g')}</p>
			</div>
		</div>
		...
		{#if lEscalado !== undefined}
			<div class="rounded-lg border border-border/50 bg-secondary/10 p-3 space-y-2">
				<div class="flex items-center gap-2">
					...
					<span class="text-xs font-semibold">Análogo escalado (h)</span>
				</div>
				<div class="rounded-md bg-background/60 p-2 text-center">
					<p class="font-mono text-[11px]">{formatearFormulaSigmoidal({ ...curvaReferencia.parametros, L: lEscalado }, 'h')}</p>
				</div>
                <p class="text-[10px] text-muted-foreground pl-5">L: {curvaReferencia.parametros.L.toFixed(1)} → {lEscalado.toFixed(1)} mm</p>
			</div>
		{/if}
	{/if}

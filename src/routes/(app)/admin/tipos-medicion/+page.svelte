					...
					<Card.Content class="pt-6">
						<form method="POST" action="?/create" use:enhance={handleAction} class="flex flex-col sm:flex-row gap-4 items-end">
							<div class="w-full sm:flex-1 space-y-2">
								<label for="new_codigo" class="text-[10px] font-medium uppercase tracking-wider text-muted-foreground block font-body">Código identificador (Ej: TEMP_AGUA)</label>
								<input
									id="new_codigo"
									name="codigo"
									type="text"
									...
									required
									class="w-full h-10 rounded-lg bg-background border border-border/60 px-3 text-sm font-body focus:border-ocean-light focus:outline-none focus:ring-2 focus:ring-ocean-light/20 transition-all uppercase"
								/>
							</div>
							<div class="w-full sm:w-48 space-y-2">
								<label for="new_unidad" class="text-[10px] font-medium uppercase tracking-wider text-muted-foreground block font-body">Unidad Base (Ej: ºC, kg)</label>
								<input
									id="new_unidad"
									name="unidadBase"
									type="text"
					...
					<table class="w-full text-sm font-body">
						<thead>
							<tr class="border-b border-border/40 bg-secondary/30">
								...
								<th class="text-left py-3 px-4 text-xs font-medium uppercase tracking-wider text-muted-foreground">Código</th>
								<th class="text-left py-3 px-4 text-xs font-medium uppercase tracking-wider text-muted-foreground">Unidad Base</th>
						...
						<tbody>
							{#if data.tipos.length === 0}
								<tr>
									<td colspan="4" class="text-center py-8 text-muted-foreground font-body">
										No hay tipos de medición registrados. Pense que la base de datos se sembraba automáticamente, revisa eso.
									</td>
								</tr>
							{:else}
								{#each data.tipos as tipo}
										...
										<tr class="border-b border-border/20 bg-ocean-light/[0.03]">
											<td colspan="4" class="py-3 px-4">
												<form method="POST" action="?/update" use:enhance={handleAction} class="flex flex-wrap items-end gap-3">
													...
													<div class="flex items-center w-12 text-xs text-muted-foreground px-2">#{tipo.id}</div>
													...
													<div class="flex-1 min-w-[150px] space-y-1 block">
														<input
															...
															required
															class="w-full h-9 rounded-lg bg-background border border-border/60 px-3 text-sm font-body focus:border-ocean-light focus:outline-none focus:ring-2 focus:ring-ocean-light/20 uppercase"
														/>
													</div>
													
													<div class="w-32 space-y-1 block">
														<input
															name="unidadBase"
															type="text"
									...
									{:else}
										...
										<tr class="border-b border-border/20 hover:bg-secondary/20 transition-colors group">
											<td class="py-3 px-4 tabular-nums text-muted-foreground">#{tipo.id}</td>
											<td class="py-3 px-4 font-medium text-foreground tracking-wide font-mono text-[13px]">{tipo.codigo}</td>
											<td class="py-3 px-4">
												<span class="px-2 py-0.5 rounded text-xs font-medium bg-secondary text-secondary-foreground border border-border/70 border-b-2">
													{tipo.unidadBase}
												</span>
											</td>
											<td class="py-3 px-4">

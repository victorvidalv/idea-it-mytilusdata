...
export const load: PageServerLoad = async ({ locals }) => {
    ...
    const [centrosResult, ciclosResult, medicionesResult] = await Promise.all([
        ...
        canViewAll
            ...
            : db.select({ total: count() }).from(ciclos).where(and(eq(ciclos.userId, userId!), eq(ciclos.activo, true))).get(),

        // Contar mediciones del mes
        canViewAll
            ? db.select({ total: count() }).from(mediciones).where(gte(mediciones.fechaMedicion, startOfMonth)).get()
            : db.select({ total: count() }).from(mediciones).where(and(eq(mediciones.userId, userId!), gte(mediciones.fechaMedicion, startOfMonth))).get()
    ]);

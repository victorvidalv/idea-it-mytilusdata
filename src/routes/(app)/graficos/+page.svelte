<script lang="ts">
	...
	$: stats = (() => {
		if (registrosFiltrados.length === 0) {
			return { total: 0, porTipo: [] as { codigo: string; unidad: string; promedio: number; min: number; max: number; cuenta: number }[] };
		}

		const porTipo = tiposActivos.map((tipo) => {
			const valores = registrosFiltrados
				.filter((r) => r.tipoId === tipo.id)
				.map((r) => r.valor);

			if (valores.length === 0)
				return { codigo: tipo.codigo, unidad: tipo.unidadBase, promedio: 0, min: 0, max: 0, cuenta: 0 };
			...
			<Card.Content>
				<div class="grid gap-4 md:grid-cols-3">
					...
					<div class="space-y-1.5">
						<label
							for="centro-select"
							class="font-body text-xs font-medium text-muted-foreground"
						>
							Centro de Cultivo
						</label>
						<select
					...
					<div class="space-y-1.5">
						...
						<div class="flex flex-wrap gap-2 pt-1">
							{#each data.tipos as tipo, i (tipo.id)}
								...
								>
									<div
										class="h-2.5 w-2.5 rounded-full transition-opacity {selectedTipoIds.has(
											tipo.id
										)
											? 'opacity-100'
											: 'opacity-30'}"
						...
						<Card.Root class="border-border/50 transition-colors duration-300 hover:border-border">
							...
							<Card.Content>
								<div class="font-display text-xl text-foreground">
									{ultimaMedicion.valor} {ultimaMedicion.tipoUnidad}
								</div>
								<p class="mt-1 font-body text-xs text-muted-foreground">
							...
							<Card.Content>
								...
								<div class="mt-1.5 flex items-center gap-3 font-body text-xs text-muted-foreground">
									<span class="flex items-center gap-1">
										<svg class="h-3 w-3 text-teal-glow" fill="none" viewBox="0 0 24 24" stroke="currentColor">
											<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7" />
										</svg>
										Max: {tipoStat.max}
									</span>
									<span class="flex items-center gap-1">
										<svg class="h-3 w-3 text-destructive" fill="none" viewBox="0 0 24 24" stroke="currentColor">

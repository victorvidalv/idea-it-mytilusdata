...
import { mediciones, apiKeys, ciclos, lugares, tiposRegistro, origenDatos } from '$lib/server/db/schema';
...
export async function GET({ request }) {
    ...
    try {
        ...
        const userRegistros = await db.select({
            id: mediciones.id,
        ...
        }).from(mediciones).where(eq(mediciones.userId, apiKeyRecord.userId));
